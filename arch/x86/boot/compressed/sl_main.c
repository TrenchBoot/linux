...
static void sl_skinit_validate_buffers(void *bootparams)
{
	...
	/*
	 * Ensure the event log buffer doesn't overlap with the bootparams or
	 * the kernel image.
	 */

	if (sl_check_buffer_overlap(evtlog_base, evtlog_end,
				    bootparams, bootparams + PAGE_SIZE))
		sl_skinit_reset();
...
asmlinkage __visible void sl_main(void *bootparams)
{
	 ...
	 * through the Secure Launch entry point and the CPU is in a suitable
	 * mode.
	 */
	 ...
	 * MLE (the kernel) and proper PMR coverage before using it.
	 *
	 * On an SKINIT launch, the boot params have to be used here to find the
	 * base and extent of the launched kernel. These values can then be
	 * used to make sure the logging buffer does not overlap the boot params or
	 * the kernel.
	 */
	if (sl_cpu_type & SL_CPU_INTEL)
	...
	/* Find the TPM hash algorithms recorded in the event log */
	if (tpm_log_ver == SL_TPM2_LOG)

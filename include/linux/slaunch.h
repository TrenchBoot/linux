...
/*
 * TPM event log structures defined by the TXT specification are derived from
 * the TCG documentation. For TXT this is set up as the container header. On AMD
 * this header is embedded into vendor information part of the TCG spec ID
 * header.
 */
...
struct skinit_sl_header {
	/* These are defined in AMD64 Architecture Programmer’s Manual, Vol. 2 */
	u16 skl_entry_point;
	u16 measured_prefix_size;
	/* These are specific to SKL (secure-kernel-loader) */
	u16 skl_info_offset;
